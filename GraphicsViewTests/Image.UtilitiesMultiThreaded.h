            ...
            [ sourceData, destData, sourceBPL, dstBPL, minX, minY ]( cRange iOff, cRange iRange )
        {
            ...
            for( int y = startY; y < endY; ++y )
            {
                ...
                for( int x = startX; x < endX; ++x )
                {
                    ...
                    BlendPixelNormalF( &destScanline, *(sourceScanline+2), *(sourceScanline+1), *(sourceScanline), alpha );
                    sourceScanline += 4;
                }
            }
        },
                                   ...
                                   const QTransform& iTransform, const QPoint& iOrigin )
{
    ...
    if( xScaleFactor >= 1.0 || yScaleFactor >= 1.0 )
    {
        MTBlendImageNormalF( iInput, iInputWidth, iInputHeight, iOutput, iOutputWidth, iOutputHeight, QPoint( minX, minY ) );
        return;
    }
